const {
  ...
  /**
   * Service function which holds business logic to fetch unique values of each insights schema fields.
   */
  const getPropertyUniqueValues = async (orgId, properties) => {
    const aggregateQuery =  getUniqueValuesQuery(Entities.insights, properties);
    console.log(aggregateQuery);
    const elasticClient =await GetElasticClient();
    const elasticResponse = await elasticClient.search({
      index: ElasticIndices.insights,
      body: {
        aggs: aggregateQuery
      }
    });
  ...
  /**
   * Service function which holds business logic to fetch paginated and filtered insights data.
   * @param {string} orgId
   * @param {*} options
   * @returns
   */
