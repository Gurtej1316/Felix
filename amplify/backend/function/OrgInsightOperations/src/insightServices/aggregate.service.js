...
/**
 * Service function which holds business logic to perform aggregated queries on insights index.
 * @param {string} orgId
 * @param {*} options
 * @returns
 */
const getAggregatedResults = async (orgId, filterOptions, periodFilter, aggregateOptions) => {
  ...
  if(filterOptions){
    ...
    searchQuery = getSearchQuery(Entities.insights, filterOptions);
    }
  }
 
  const aggregationQuery = getAggregationQuery(Entities.insights, aggregateOptions);
  const elasticQuery = {
