...
/**
 * Service function which holds business logic to perform aggregated queries on action index.
 * @param {string} orgId
 * @param {*} options
 * @returns
 */
const getAggregatedResults = async (orgId, filterOptions, periodFilter, aggregateOptions) => {
  ...
  if(filterOptions){
    ...
    searchQuery = getSearchQuery(Entities.actions, filterOptions);
    }
  }
 
  const aggregationQuery = getAggregationQuery(Entities.actions, aggregateOptions);
  const elasticQuery = {
